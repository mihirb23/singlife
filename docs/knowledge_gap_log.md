...
|---|--------------|-------|--------|------------------------|--------|
| 1 | Step 8H — RCS checks | ICD code pass/fail list is not included in the SOP. AI cannot determine whether specific claim ICD codes should pass or be referred to UW. | Cannot complete Jet UW evaluation when Claim Ind = Y | Need the full ICD code classification list (which codes pass, which refer to UW) | **Partially Resolved** — 26 ICD codes extracted from NB.MASTER with risk tiers (see `ref_icd_codes.txt`). Still need SME confirmation on pass/fail thresholds. |
...
| 3 | Step 7 — Follow-up codes | F45 (Premium mismatch), C09 (GNS re-screening), CSL (Clarification required) now documented from 30 Mar update. AT3 (Additional requirement) also known. But other codes may exist in UAT data that are still undocumented | AI can handle the 4 known codes but may encounter unknown ones | Need confirmation this is the complete list, or share any additional FUP codes | **Resolved** — 93 unique follow-up codes extracted from NB.MASTER with frequencies and categories (see `ref_followup_codes.txt`). Full dictionary now in knowledge base. |
